...
fn director_bootstrap(
    ...
    runtime.spawn_rng = DetRng::from_seed(spawn_seed(&params));
...
fn drive_wheel_state(
    ...
    let current_tick = state.leg_tick;
        ...
        match event.action {
            ...
            WheelAction::Pause(value) => {
                ...
                match (value, state.status) {
                    (true, LegStatus::Running) => {
                        state.status = LegStatus::Paused;
                    }
                    (false, LegStatus::Paused) => {
                        state.status = LegStatus::Running;
                    }
                    _ => {}
                }
...
fn mission_seed(params: &LegParameters, index: usize, mission: &str) -> u64 {
    ...
    hasher.update(params.world_seed.as_bytes());
    hasher.update(params.rng_salt.as_bytes());
    hasher.update(params.link_label.as_bytes());
