...
pub fn get_drift_offset_mm(
    weather: Weather,
    config: &WeatherConfig,
    tick: u64,
    _route_id: Option<u16>,
) -> i32 {
...
mod tests {
    ...
    fn drift_stability() {
        ...
        assert_eq!(
            offset1, offset2,
            "Drift should be deterministic for same inputs"
        );
    ...
    fn drift_zero_for_clear_weather() {
        ...
        assert!(
            drift_rains.abs() > 0,
            "Rains should have non-zero drift offset (can be positive or negative)"
        );
        assert!(
            drift_windy.abs() > 0,
            "Windy should have non-zero drift offset (can be positive or negative)"
        );
    }
}
