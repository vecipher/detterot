...
use crate::world::schema::{Board, BoardDims, Cover, CoverKind, Direction, Point, Rectangle, SpawnPoints, Wall, Zones};
...
fn generate_walls(rng: &mut Xoshiro256PlusPlus, w: u32, h: u32) -> Vec<Wall> {
    ...
    for _ in 0..wall_count {
        ...
        let len = len.min(if is_horizontal {
            w as i32 - x - 1
        } else {
            h as i32 - y - 1
        }.max(1) as u32);
...
mod tests {
    ...
    fn boardgen_determinism() {
        ...
        assert_eq!(canonical_json(&board1).unwrap(), canonical_json(&board2).unwrap());
        ...
        assert!(hash1 != hash3 || hash1 != hash4 || hash3 != hash4, 
    ...
    fn different_seeds_produce_different_boards() {
        ...
        assert_ne!(hash1, hash2, "Different seeds should produce different boards");
    }
}